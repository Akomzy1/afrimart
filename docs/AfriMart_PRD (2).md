...
| **SEL-2** | Seller verification and food-safety attestation, appropriate to the seller type, required before any seller of any type — store, home-based or new entrant — can list to the public or be assigned an order by the routing engine. An unverified seller's lines are withheld as blocked-by-verification, reported distinctly from out-of-stock.                                                                  | 1         |
